...
# sqlc yaml file
SQLC_YAML ?= ./internal/storage/sqlc.yaml

.PHONY : postgresup postgresdown psql createdb teardown_recreate generate

postgresup:
	docker run --name test-postgres -v $(PWD):/usr/share/vw -e POSTGRES_PASSWORD=$(DB_PWD) -p 5432:5432 -d $(DB_NAME)
...
createdb:
	docker exec -it test-postgres psql $(PSQLURL) -c "\i /usr/share/vw/internal/storage/db/schema.sql"
